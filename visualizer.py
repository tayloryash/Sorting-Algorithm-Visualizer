...
def paintData(data, colorArray,c):
    ...
    for i, height in enumerate(data):
        ...
        canvas2.create_text(240, 50, text=c, fill="black", font='Helvetica 11 ')
...
### User interface here ###
portion_UI = Frame(root, width=300, height=400, bg=WHITE)
portion_UI.grid(row=0, column=0, padx=10, pady=5)

# dropdown to select sorting algorithm
l1 = Label(portion_UI, text="Algorithm:", bg=BLUE)
...
l2 = Label(portion_UI, text="Sorting Speed:", bg=BLUE)
...
# Array size
l3 = Label(portion_UI, text="Array Size:", bg=BLUE)
...
# Order
l4 = Label(portion_UI, text="Order of Sorting", bg=BLUE)
l4.grid(row=1, column=2, padx=10, pady=5, sticky=W)
order_menu = ttk.Combobox(portion_UI, state="readonly", textvariable=sorting_order, values=order_list)
...
# sort button
b1 = Button(portion_UI, text="Sort", command=sort, bg=LIGHT_GRAY)
b1.grid(row=2, column=1, padx=5, pady=5)

# button for generating array
b3 = Button(portion_UI, text="Generate Array", command=generateData, bg=LIGHT_GRAY)
...
canvas2 = Canvas(root, width=300, height=200, bg=WHITE, highlightbackground=BLUE, highlightthickness=2)
